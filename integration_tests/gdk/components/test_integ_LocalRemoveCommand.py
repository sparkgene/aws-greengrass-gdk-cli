...
class LocalRemoveCommandTest(TestCase):
    ...
    def test_LocalRemoveCommand(self, mock_run):
        mock_result = Mock()
        ...
        comm = LocalRemoveCommand({})
        comm.run()
        mock_run.assert_called_once_with(
            ["sudo", "/greengrass/v2/bin/greengrass-cli",
             "deployment", "create", "--remove", "abc"],
            shell=False, capture_output=True, text=True, timeout=60
        )

    @patch("subprocess.run")
    def test_LocalRemoveCommand_with_command_args(self, mock_run):
        mock_result = Mock()
        mock_result.returncode = 0
        mock_result.stdout = "File transferred successfully"
        mock_run.return_value = mock_result
        ...
        comm = LocalRemoveCommand(command_args)
        comm.run()
        mock_run.assert_called_once_with(
            ["ssh", "-p", "1234", "-i", "/args/path/to/key", "my-user@my-host",
             "sudo", "/args/path/to/greengrass/bin/greengrass-cli",
             "deployment", "create", "--remove", "abc"],
            shell=False, capture_output=True, text=True, timeout=60
        )

    @patch("subprocess.run")
    def test_LocalRemoveCommand_on_remote(self, mock_run):
        mock_result = Mock()
        mock_result.returncode = 0
        mock_result.stdout = "File transferred successfully"
        mock_run.return_value = mock_result
        ...
        mock_run.assert_called_once_with(
            ["ssh", "-p", "8022", "-i", "/path/to/key", "remote-user@remote_host",
             "sudo", "/path/to/greengrass/bin/greengrass-cli",
             "deployment", "create", "--remove", "abc"],
            shell=False, capture_output=True, text=True, timeout=60
        )
